...
import 'package:awjq/pages/community/community_tag_detail_child.dart';
...
class CommunityTagDetail extends BaseWidget {
  CommunityTagDetail({Key key, this.topic_id, this.offsetBack, this.type = ""})
      : super(key: key);
...
class _CommunityTagDetailState extends BaseWidgetState<CommunityTagDetail> {
  ...
  _getTopDetail() {
    communityTopicsDetail(topic_id: widget.topic_id, type: widget.type)
        .then((res) {
      if (res.status == 1) {
        topic = res.data;
