...
import 'package:awjq/pages/community/community_focus.dart';
import 'package:awjq/pages/community/community_new.dart';
import 'package:awjq/pages/community/community_recommend.dart';
...
class FollowedListPage extends StatefulWidget {
  FollowedListPage({Key key, this.type}) : super(key: key);
  final String type;
...
class _FollowedListPageState extends State<FollowedListPage> {
  ...
  _getData() {
    focusTops(page: page, last_ix: last_ix, type: widget.type).then((res) {
      if (res.data == null) {
        networkErr = true;
        setState(() {});
        return;
      ...
      child: Column(
        children: [
          GestureDetector(
            behavior: HitTestBehavior.translucent,
            onTap: () {
              context.push("/communitytagdetail/${e["id"]}/sc");
            },
            child: Container(
              height: _w / 170 * 85,
              child: Stack(
