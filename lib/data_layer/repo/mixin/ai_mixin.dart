...
mixin _AI on _BaseAppRepo implements AIDomain {
  ...
  AsyncResult<List<AIModel>?> aIMyFace({
    ...
    page: page,
    limit: limit,
  ).deserializeJsonListBy((e) => e.map(AIModel.fromJson).toList()).guard;
  ...
  AsyncResult aIDelFace({
    required int id}) =>
      _aiService.aIDelFace(
          id: id).deserialize().guard;

  @override
  AsyncResult aIDelStrip({
    required int id}) =>
      _aiService.aIDelStrip(
          id: id).deserialize().guard;
