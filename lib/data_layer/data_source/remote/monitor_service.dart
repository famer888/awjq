...
class MonitorService extends BaseService {
  ...
  /// 监控购买
  AsyncJson getMonitorBuy({required int id}) =>
      post('/buy', data: {'id': id});

  /// 监控收藏列表
  AsyncJson getMonitorListFavorite({required int page,
    required int limit}) =>
      post('/list_favorite', data: {'page': page, 'limit': limit});

  /// 监控已购买列表
  AsyncJson getMonitorListBuy({required int page,
    required int limit}) =>
      post('/list_buy', data: {'page': page, 'limit': limit});

  /// 监控评论
  AsyncJson getMonitorComment({
